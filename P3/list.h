...
/**
 * P_ele_print type definition: pointer to a function that prints a List element
 * to an output stream. It is assumed that this function returns: (1) the number
 * of characters printed upon success; or (2) a negative number in case of
 * error.
 */
typedef int (*P_ele_print)(FILE *, const void *);
/**
 * P_ele_cmp type definition: pointer to a function that compares two elements
 * of a List It returns an integer less than, equal to, or greater than zero if
 ...
 * List.
 *
 * Inserts into the front position a reference of the element received as
 * argument.
 *
 * @param pl Pointer to the List.
 ...
 * List.
 *
 * Inserts into the back position a reference of the element received as
 * argument.
 *
 ...
 * List must be traversed in order to obtain its size. So this operation is
 * linear in the number of List elements.
 *
 * @param pl Pointer to the List.
 *
 * @return Returns the number of elements in the List, or -1 if the List is
 * NULL.
 */
 ...
 * element, also printing the size of this list at the beginning and an
 * additional blank space between elements. Any other desired format must be
 * included in the P_ele_print function.
 *
 * @param fp Output stream.
 * @param pl Pointer to the List.
 * @param f A pointer to the function that must be used to print the list
 * elements.
 *
 ...
 * the function P_ele_print is well constructed, this means that, upon
 * successful return, this function returns the number of characters printed,
